...
Left2RightSwip:%d Right2LeftSwip:%d Up2DownSwip:%d Down2UpSwip:%d\n",
	...
	if ((gesture == DouTap && double_tap_enable) || (gesture == RightVee && right_arrow_enable)
		...
		|| (gesture == Sgestrue && letter_s_enable) || (gesture == Wgestrue && letter_w_enable) || (gesture == Mgestrue && letter_m_enable)
...
static ssize_t tp_gesture_write_func(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	letter_s_enable = (buf[0] & BIT8)?1:0;//"S"
	letter_m_enable  = (buf[0] & BIT9)?1:0; //"M"
	letter_w_enable  = (buf[0] & BITA)?1:0; //"W"

	if(double_tap_enable||letter_o_enable||down_arrow_enable||up_arrow_enable||left_arrow_enable\
        ||right_arrow_enable||double_swipe_enable||letter_s_enable||letter_w_enable||letter_m_enable)
	{
		ts->gesture_enable = 1;
	}
	else
...
static void gesture_enable(struct synaptics_ts_data *ts)
{
	ts->gesture_enable = double_tap_enable || letter_o_enable || down_arrow_enable || up_arrow_enable ||
		...
		letter_s_enable || letter_w_enable || letter_m_enable || left_swipe_enable || 
		down_swipe_enable || up_swipe_enable ? 1 : 0;
}
...
static int init_synaptics_proc(void)
{
	...
	prEntry_tmp = proc_create("letter_s_enable", 0666, prEntry_tp, &letter_s_proc_fops);
	if(prEntry_tmp == NULL){
		ret = -ENOMEM;
		TPD_ERR("Couldn't create letter_s_enable\n");
	}
